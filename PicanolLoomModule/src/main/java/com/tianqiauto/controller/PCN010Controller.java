package com.tianqiauto.controller;

import com.tianqiauto.bean.PCN;
import com.tianqiauto.bean.ParamVo;
import com.tianqiauto.utils.BytesUtil;
import com.tianqiauto.utils.StringUtils;
import com.tianqiauto.utils.dispenser.AbstractBispenser;
        ...
        if(cnt > 1){//包含其他信息
            if(body.getData()[1] == 30 && body.getData()[2] == 3){//达到预选（布卷长度）时机器发送的消息 落布的消息
                switch (body.getData()[3]){
                    case 1:ParamVo.addParam(sourceId,"落布长度单位","picks"); break;
                    case 2:ParamVo.addParam(sourceId,"落布长度单位","meters"); break;
                    case 3:ParamVo.addParam(sourceId,"落布长度单位","yards"); break;
                    case 5:ParamVo.addParam(sourceId,"落布长度单位","cm"); break;
                    case 6:ParamVo.addParam(sourceId,"落布长度单位","inch"); break;
                    case 7:ParamVo.addParam(sourceId,"落布长度单位","jacquard patterns"); break;
                    case 8:ParamVo.addParam(sourceId,"落布长度单位","dobby patterns"); break;
                    case 9:ParamVo.addParam(sourceId,"落布长度单位","jacquard patterns"); break;
